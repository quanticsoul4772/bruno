...
### Multi-Agent Swarm (Separated)
The swarm CLI (`bruno-swarm`) has been moved to its own repository: `bruno-swarm`. See that project for CrewAI agent orchestration, Modelfiles, and swarm examples. Models are hosted at `rawcell/bruno-swarm-models` on HuggingFace.
