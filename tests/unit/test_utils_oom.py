...
class TestGPUMemoryInfo:
    ...
    def test_get_gpu_memory_info_with_cuda(self):
        ...
        with patch("heretic.utils.torch.cuda.is_available", return_value=True), \
             patch("heretic.utils.torch.cuda.get_device_properties", return_value=mock_props), \
             patch("heretic.utils.torch.cuda.memory_reserved", return_value=2_000_000_000), \
             patch("heretic.utils.torch.cuda.memory_allocated", return_value=1_500_000_000):
    ...
    def test_get_gpu_memory_info_handles_exception(self):
        ...
        with patch("heretic.utils.torch.cuda.is_available", return_value=True), \
             patch("heretic.utils.torch.cuda.get_device_properties", side_effect=RuntimeError("GPU error")):
